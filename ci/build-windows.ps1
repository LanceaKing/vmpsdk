...
if ($Kind -eq 'ddk' -and $Arch -ne 'x86') { throw 'The original DDK project supports only x86' }
...
if ($Kind -eq 'msvc') {
    ...
    $source = Join-Path $work 'Examples/Licensing/DDK'
    $dest = Join-Path $out 'licensing-ddk-x86'
    ...
    Copy-Item (Join-Path $work 'Lib/Windows/VMProtectDDK32.sys') $dest
